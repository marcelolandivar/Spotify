...
def _authorization_code(conf):
    """"Try to open a file .pytify in the current directory which has the
    refresh access token that is going to be used to refresh the access token
    ...
    try:
        ...
        raise IOError(('It seems you have not authorize the application yet.'
...
def _refresh_access_token(auth_key, refresh_token):
    ...
    if not response.ok:
        ...
        raise SpotifyException(error_description)
...
def _client_credentials(conf):
    ...
    expires_in = content.get('expires_in', None)
    scope = content.get('scope', None)
    # The parameter set to None is because we do not want
    # the service to return a refresh_token when the type of
